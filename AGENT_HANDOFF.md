...
**"Tour-Spiel" section** (`#concert-snippets-section`): looks up `ARTIST_CONTEXT[m.creator]?.[concertYear]`. If found, renders blurb text and quote blocks. Both blurb and quotes are optional.
...
| `js/artist-context.json` | Static JSON. Keys: artist name → year (string) → `{ blurb: string, quotes: [{text, attr}][] }`. Used on concert detail for "Tour-Spiel" section. Curated manually. |
